...
gcloud run deploy tapandstamp-admin-staging \
  ...
  --min-instances 0 \
  --max-instances 10 \
  --set-env-vars "NODE_ENV=staging,NEXT_PUBLIC_API_URL=$API_CUSTOM_URL,NEXT_PUBLIC_BASE_URL=$ADMIN_URL,NEXT_PUBLIC_ADMIN_URL=$ADMIN_URL,NEXT_PUBLIC_SUPABASE_URL=https://pofacqesahciilruwlpp.supabase.co,APPLE_PASSKIT_CERT_PATH=/secrets/passkit/cert.p12,APPLE_WWDR_CERT_PATH=/secrets/wwdr/cert.cer,APPLE_APNS_KEY_PATH=/secrets/apns/key.p8,CONTENTFUL_ENVIRONMENT=staging" \
  --set-secrets "SUPABASE_SERVICE_ROLE_KEY=tapandstamp-staging-supabase-service-key:latest,NEXT_PUBLIC_SUPABASE_ANON_KEY=tapandstamp-staging-supabase-anon-key:latest,APPLE_PASS_TYPE_ID=tapandstamp-apple-pass-type-id:latest,APPLE_TEAM_ID=tapandstamp-apple-team-id:latest,APPLE_PASSKIT_CERT_PASSWORD=tapandstamp-passkit-cert-password:latest,APPLE_APNS_KEY_ID=tapandstamp-apns-key-id:latest,PASSKIT_AUTH_SECRET=tapandstamp-passkit-auth-secret:latest,/secrets/passkit/cert.p12=tapandstamp-passkit-cert:latest,/secrets/wwdr/cert.cer=tapandstamp-wwdr-cert:latest,/secrets/apns/key.p8=tapandstamp-apns-key:latest,CONTENTFUL_SPACE_ID=contentful-space-id:latest,CONTENTFUL_DELIVERY_TOKEN=contentful-staging-delivery-token:latest,CONTENTFUL_PREVIEW_TOKEN=contentful-staging-preview-token:latest,CONTENTFUL_PREVIEW_SECRET=contentful-staging-preview-secret:latest,CONTENTFUL_REVALIDATE_SECRET=contentful-staging-revalidate-secret:latest"
